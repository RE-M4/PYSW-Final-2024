...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  exportToExcel(): void {
    ...
    const filename: string = 'Grupo_5_PySWEB_2024_TablaTurnos.pdf';
      ...
      (result: any) => {
        ...
        result.forEach((element: any) => { ii++;
         ...
         if (this.idTurno != null){ 
          if (this.turno._id==this.idTurno){ 
 ...
 //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  pacienteBorrado(iiddPac:string){
    let max = this.borradosPac.length;
    for (let i = 0; i < max; i++) {
      if (this.borradosPac[i]._id == iiddPac) {
        this.apPaciente = this.borradosPac[i].apellido + "_" + this.borradosPac[i].nombre;
        this.dniPaciente = this.borradosPac[i].dni;
     } }
    return this.apPaciente;
   }

 medicoBorrado(iiddMed:string){
  let max = this.borradosMed.length;
  for (let i = 0; i < max; i++) {
    if (this.borradosMed[i]._id == iiddMed) {
      this.apMedico = this.borradosMed[i].apellido + "_" + this.borradosMed[i].nombre;
      this.dniMedico = this.borradosMed[i].dni;
   } }
  return this.apMedico;
 }
 ...
 //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  obtenerMedico() {
    this.sMedico.getMedicoById(this.idMedico).subscribe(
      (result: any) => {
        this.medicoAux._id = result._id;
        this.medicoAux.apellido = result.apellido;
        this.medicoAux.nombre = result.nombre;
        this.medicoAux.dni = result.dni;
        this.medicoAux.contactos = result.contactos;
        this.medicoAux.domicilio = result.domicilio;
        this.medicoAux.matricula = result.matricula;

        console.log("__medico_encontrado::" + this.medicoAux.dni);
      },
  ...
  obtenerPaciente() {
    this.sPaciente.getPacienteById(this.idPaciente).subscribe(
      (result: any) => {
        this.pacienteAux._id = result._id;
        this.pacienteAux.apellido = result.apellido;
        this.pacienteAux.nombre = result.nombre;
        this.pacienteAux.dni = result.dni;
        this.pacienteAux.contactos = result.contactos;
        this.pacienteAux.domicilio = result.domicilio;
        this.pacienteAux.historia = result.historia;

        console.log("__paciente_encontrado::" + this.pacienteAux.dni);
      },
      (error: any) => { console.log(error); }
    )
  }
    ...
    if (this.idTurno!= "") {
      if (confirm("Estas seguro de borrar el turno?")) {
        this.sTurno.deleteTurno(this.idTurno).subscribe(
          ...
          (error: any) => { console.log(error); console.log("ERROR BORRADO Turno"); 
...
busquedaMedico() {
  ...
  ttiposBusqueda(){
       if (this.BuscaTTurno==0 || this.BuscaTTurno==1 || this.BuscaTTurno==2 
        ...
        } else {console.log("entro en else_tipo_Busqueda");
  ...
  async modificaTurno() {
    ...
    if (this.control(this.idPaciente, this.idMedico, this.fecha, this.hhora) == false) {
      try {
        // Espera a que buscaMedico() complete su tarea
       // await this.buscaMedico();
        // Espera a que buscaPaciente() complete su tarea
       // await this.buscaPaciente();
        // Luego de completar las búsquedas, continúa con el resto del código
        let medBBusca: Medico | undefined = this.listaMed.find(medico => medico._id === this.idMedico);
        let pacBBusca: Paciente | undefined = this.listaPac.find(paciente => paciente._id === this.idPaciente);
        ...
        this.turnoAux.fechaturno = fechaConvertida;
        this.turnoAux.horaturno = fechaConvertida;
        this.turnoAux.sala = this.sala;
        this.turnoAux.pagado = this.pagado;
        if (pacBBusca){this.turnoAux.paciente = pacBBusca;}
        if (medBBusca){this.turnoAux.medico = medBBusca;}
        this.turnoAux.estado = this.estadoTurno;
        this.turnoAux.enfermedad = this.enfermedad;
        this.turnoAux.precio = this.precio;
        this.turnoAux.tipoPago = this.tipoPago;
        this.turnoAux._id = this.idTurno;
  
        // Llamamos al servicio para actualizar el turno
        await this.sTurno.putTurno(this.turnoAux).toPromise();
        alert("TURNO MODIFICADO CORRECTAMENTE");
        
        define = 1;
        if (define == 1) {
          this.mostrarLast();
         await this.cargaTurno();
        }
      } catch (error) {
        console.error("Error en buscaMedico o buscaPaciente:", error);
        alert("ERROR en buscaMedico o buscaPaciente");
      }
    }
  }
  */
